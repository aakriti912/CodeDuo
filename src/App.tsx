...
const App = () => {
  ...
  const getFavoriteSpellIndexes = () => {
    return favorites.map((spell) => spell.index);
  };
  const removeFavorite = (index: string) => {
    const updatedFavorites = favorites.filter((spell) => spell.index !== index);
          ...
          <>
            ...
            <div className="fixed inset-0 flex justify-center items-center z-20">
              <div className="bg-white p-6 rounded-lg shadow-lg relative">
                ...
                >
                  <span className="text-red-500 text-5xl"> &times; </span>
                </button>
                <SpellDetails
                  spell={selectedSpell}
                  onAddFavorite={addFavorite}
                  favoriteSpellIndexes={getFavoriteSpellIndexes()}
                />
              </div>
            </div>
          </>
        )}
        <Favorites spells={favorites} removeFavorite={removeFavorite} />
      </div>
    </div>
  );
};
